    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 select-none">
      ...
      {selectedBlog ? (
        ...
        <div className="max-w-3xl mx-auto animate-fade-in">
          ...
          <h2 className={`font-serif tracking-tight font-extrabold ${
          ...
          <div className={`leading-relaxed whitespace-pre-wrap space-y-4 ${
            lightMode ? 'text-slate-700 font-medium' : 'text-slate-300'
          } ${getTextClass()}`}>
            {selectedBlog.content}
          </div>

          <div className={`mt-12 p-6 rounded-2xl border text-xs flex flex-col sm:flex-row items-center justify-between gap-4 ${
            lightMode ? 'bg-blue-50 border-blue-200' : 'bg-blue-600/5 border-blue-500/20'
          }`}>
            <div>
              <h4 className={`font-semibold mb-0.5 ${lightMode ? 'text-blue-900' : 'text-white'}`}>Ready to consult with a Portfolio Advisor?</h4>
              <p className={lightMode ? 'text-slate-650' : 'text-slate-400'}>We provide free private transport options around Nagpur corridors.</p>
            </div>
            <button
              onClick={() => {
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {BLOGS.map((blog) => (
              <div 
                key={blog.id}
                className={`border rounded-2xl overflow-hidden flex flex-col justify-between hover:shadow-xl transition-all duration-300 ${
              ...
              >
                <div>
                  ...
                  <div className="p-5">
                    ...
                    <h3 className={`font-serif font-bold text-base tracking-tight leading-snug line-clamp-2 ${
                      lightMode ? 'text-slate-900' : 'text-slate-100'
                    }`}>
                      {blog.title}
                    </h3>
