...
import { CheckCircle, HelpCircle, ArrowRight, ArrowLeft, Coins, Calculator, MapPin, Sparkles, Navigation, ShieldCheck } from 'lucide-react';
...
export default function ProjectDetailView({
  ...
  const [loanAmount, setLoanAmount] = useState<number>(Math.floor(defaultPrice * 0.8)); // default 80% loan
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          ...
          <div>
            <span className="text-[10px] font-bold uppercase tracking-widest bg-blue-600/10 text-blue-400 border border-blue-500/20 px-2.5 py-1 rounded font-mono">
              {project.type.toUpperCase()} SPECIFICATIONS
            </span>
            <h1 className={`font-serif tracking-tight font-black mt-3 ${
              lightMode ? 'text-slate-900' : 'text-slate-100'
            } ${getHeadingClass()}`}>
              {project.name}
            </h1>
            <p className="text-slate-500 text-xs flex items-center gap-1.5 mt-1">
              <MapPin className="w-4 h-4 text-red-500 shrink-0" /> {project.location} | Development Zone: {project.mapHotspot}
            </p>
          </div>
          ...
          {/* Slogans description */}
          <div>
            <h3 className={`font-serif font-bold text-lg mb-3 ${lightMode ? 'text-slate-900' : 'text-slate-100'}`}>
              Overview & Structural Masterplan
            </h3>
            <p className={`leading-relaxed ${lightMode ? 'text-slate-650' : 'text-slate-350'} ${getTextClass()}`}>
              {project.description}
            </p>
          </div>
          ...
          <div className={`p-5 rounded-2xl border ${
            lightMode ? 'bg-slate-50 border-slate-200 shadow-sm' : 'bg-slate-950/40 border-slate-850'
          }`}>
            ...
            <ul className="space-y-2 text-xs">
              {project.details.map((det, index) => (
                <li key={index} className="flex items-start gap-2.5">
                  ...
                  <span className={`${lightMode ? 'text-slate-700' : 'text-slate-300'}`}>{det}</span>
                </li>
              ))}
            </ul>
          </div>
                ...
                <tbody>
                  {project.specs.map((spec, i) => (
                    ...
                    >
                      ...
                      <td className={`px-4 py-3 font-medium ${lightMode ? 'text-slate-800' : 'text-white'}`}>
                        {spec.value}
                      </td>
                    </tr>
                  ))}
              ...
              {project.highlights.map((item, idx) => (
                <div 
                  ...
                  className={`border rounded-xl p-3.5 flex items-center gap-3 ${
                    lightMode ? 'bg-[#f4f7fa] border-slate-250/50' : 'bg-[#091522] border-slate-900'
                  }`}
                >
                  <CheckCircle className="w-5 h-5 text-blue-500 shrink-0" />
                  <span className={`font-semibold ${lightMode ? 'text-slate-850' : 'text-slate-100'}`}>{item}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 text-xs">
              {project.amenities.map((am, idx) => (
                <div 
                  key={idx} 
                  className={`border rounded-lg px-3 py-2.5 text-left flex items-center gap-2 ${
                    lightMode ? 'bg-white border-slate-205' : 'bg-slate-950 border-slate-850'
                  }`}
                >
                  <span className="w-1.5 h-1.5 rounded-full bg-blue-500 shrink-0" />
                  <span className="text-slate-400 font-medium truncate">{am}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          ...
          <div className="bg-slate-950 border border-slate-850 rounded-2xl overflow-hidden p-3 shadow-lg">
            ...
            <div className="grid grid-cols-2 gap-2 mt-2 text-center text-xs font-mono font-bold text-slate-400">
              <div className="bg-slate-900 border border-slate-800 p-2.5 rounded-xl">
                <span className="text-[9px] uppercase tracking-wider text-slate-500 block">Pricing Scale</span>
                <span className="text-red-400 text-sm">{project.priceRange}</span>
              </div>
              <div className="bg-slate-900 border border-slate-800 p-2.5 rounded-xl">
                <span className="text-[9px] uppercase tracking-wider text-slate-500 block">Sizing Range</span>
                <span className="text-white text-sm">{project.acres !== 'Boutique Project' ? `${project.acres} Acres` : 'Boutique flat'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#003B72] text-white p-6 rounded-2xl border border-blue-500/20 shadow-xl relative overflow-hidden">
            ...
            <h4 className="font-serif font-bold text-lg mt-2 tracking-tight">Request Complete Legal Folder</h4>
            <p className="text-xs text-blue-250 mt-1.5 leading-relaxed">
              Have our relationship executives dispatch the complete legal folder, release letters, layout map layouts, and tax indices directly to your email in under 30 minutes.
            </p>

            <button
              onClick={() => openLeadModal(project.slug, `Please share the NMRDA sanctioned legal brochure and release letters regarding ${project.name}.`)}
          ...
          <div className={`border rounded-2xl p-5 ${
            lightMode ? 'bg-slate-105 border-slate-200' : 'bg-slate-950 border-slate-850'
          }`}>
            <div className={`flex items-center gap-1.5 border-b pb-3 mb-4 ${
              ...
              <div>
                <h4 className={`text-sm font-serif font-bold ${lightMode ? 'text-slate-900' : 'text-slate-100'}`}>
                  EMI Mortgage Estimator
                </h4>
                <span className={`text-[9px] font-mono uppercase tracking-wider ${lightMode ? 'text-slate-550' : 'text-slate-400'}`}>
            ...
            <div className="flex flex-col gap-4 text-xs select-none">
              {/* Slit Amount */}
              <div>
                <div className={`flex justify-between font-medium mb-1 font-mono text-[10px] uppercase ${
                  lightMode ? 'text-slate-600' : 'text-slate-400'
                }`}>
                  <span>Financing Loan Volume</span>
                  <span className={`font-bold px-1.5 py-0.2 rounded font-mono text-xs ${
                    lightMode ? 'text-slate-900 bg-slate-200/60' : 'text-white bg-slate-900'
                  }`}>
              ...
              <div className={`border rounded-xl p-4 text-center mt-2 relative overflow-hidden ${
                lightMode ? 'bg-[#E2E8F0]/40 border-slate-250 shadow-sm' : 'bg-slate-900/90 border-slate-800'
              }`}>
                ...
                <p className="text-[9px] text-slate-500 mt-1 lines-normal">
                  Calculation represents monthly amortization schedules. Elite private banks assist 80% financing on Melbourne, Canberra, and Shraddha Avenue plots.
                </p>
              </div>
