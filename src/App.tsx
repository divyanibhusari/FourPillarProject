    ...
    <div 
      className={`min-h-screen transition-all ${getTextHeightClass()} ${
        accessibilityHighContrast 
          ...
          : (lightMode ? 'bg-[#F8FAFC] text-slate-800' : 'bg-[#091522] text-slate-300')
      }`}
    >
        ...
        {currentRoute === 'projects' && (
          <div className="pt-24 pb-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-16">
              ...
              <h1 className={`text-3xl sm:text-4xl font-serif tracking-tight font-black mt-2 ${
                lightMode ? 'text-slate-900' : 'text-white'
              }`}>
                Active & Upcoming Gated Townships
              </h1>
              <p className={`italic mt-1 font-serif text-sm ${
            ...
            <div className="flex flex-col gap-12 mt-8">
              {PROJECTS.map((proj, idx) => (
                <div 
                  key={proj.slug}
                  className={`flex flex-col lg:flex-row rounded-3xl overflow-hidden border transition-all ${
                    lightMode ? 'bg-white border-slate-200 shadow-md' : 'bg-slate-950 border-slate-850'
                  }`}
                >
                  ...
                  <div className="lg:w-1/2 p-6 sm:p-8 flex flex-col justify-between">
                    <div>
                      ...
                      <h3 className={`text-xl font-serif font-bold tracking-tight mt-1.5 ${
                        lightMode ? 'text-slate-900' : 'text-white'
                      }`}>
                        {proj.name}
                      </h3>
                      <p className={`text-xs mt-3 leading-relaxed ${
                        lightMode ? 'text-slate-600' : 'text-slate-450'
                      }`}>
                        {proj.description}
                      </p>

                      <div className={`mt-4 grid grid-cols-2 gap-4 text-xs font-mono border-t pt-4 ${
                        lightMode ? 'border-slate-100' : 'border-slate-900/10'
                      }`}>
                        {proj.specs.slice(0, 2).map((spec, sIdx) => (
                          <div key={sIdx}>
                            <span className="text-[10px] text-slate-500 uppercase block font-normal">{spec.label}</span>
                            <span className={`font-bold mt-0.5 inline-block text-xs ${
                              lightMode ? 'text-slate-905 font-extrabold' : 'text-white'
                            }`}>{spec.value}</span>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className={`mt-8 pt-4 border-t flex justify-between items-center shrink-0 ${
                      lightMode ? 'border-slate-100' : 'border-slate-900/10'
                    }`}>
                      <button
            ...
            {/* Direct Map explore callout */}
            <div className={`mt-12 p-6 rounded-3xl border text-center relative overflow-hidden ${
              lightMode ? 'bg-white border-blue-200 shadow-md' : 'bg-slate-950 border-slate-850'
            }`}>
              <h4 className={`text-sm font-bold ${
                lightMode ? 'text-slate-900' : 'text-white'
              }`}>Uncover Individual Plot Sizing Metrics Dynamically</h4>
              <p className={`text-xs mt-1 max-w-md mx-auto ${
                lightMode ? 'text-slate-500' : 'text-slate-500'
              }`}>We plotted comprehensive coordinates. Load the Map Explorer to check which bounds remains unsold inside Kaldongri.</p>
              <button
                onClick={() => handleNavigate('map')}
                className="bg-blue-600 hover:bg-blue-500 text-white font-mono text-[10px] font-bold py-2 px-5 rounded-xl uppercase tracking-wider mt-4"
              >
                Load Layout Sandbox Map
              </button>
            </div>
          </div>
        )}
      ...
      {/* Slide-in real-time sales tracker notification tray */}
