...
export default function InteractiveMap({ onSelectProject, openLeadModal, accessibilityTextSize, lightMode }: InteractiveMapProps) {
  ...
  return (
    <div 
      ...
      className={`border rounded-3xl p-6 lg:p-8 shadow-2xl relative overflow-hidden transition-all duration-300 ${
        lightMode 
    ...
    >
      ...
      {/* Header */}
      <div className={`flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 border-b pb-6 relative z-10 ${
        ...
        <div>
          ...
          <h3 className={`font-serif tracking-tight font-bold ${
            lightMode ? 'text-slate-900' : 'text-slate-100'
          } ${getHeadingClass()}`}>
            South Nagpur Premium Expansion Corridor
          </h3>
          <p className={`text-xs mt-1 ${lightMode ? 'text-slate-600' : 'text-slate-400'}`}>
          ...
          {PROJECTS.map((proj) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                selectedMapProject?.slug === proj.slug
                  ? 'bg-blue-600 text-white shadow-lg'
                  : (lightMode ? 'bg-slate-100 text-slate-700 hover:bg-slate-200' : 'bg-slate-800 text-slate-300 hover:bg-slate-700')
              }`}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10">
        ...
        <div className={`lg:col-span-5 rounded-2xl border p-4 relative flex flex-col justify-between overflow-hidden group min-h-[360px] lg:min-h-[420px] ${
          lightMode ? 'bg-slate-50 border-slate-200' : 'bg-slate-950 border-slate-800'
        }`}>
          {/* Map Grid Pattern background */}
          <div className={`absolute inset-0 bg-[linear-gradient(to_right,#ffffff03_1px,transparent_1px),linear-gradient(to_bottom,#ffffff03_1px,transparent_1px)] bg-[size:24px_24px] ${
            lightMode ? 'opacity-30' : ''
          }`} />
          
          {/* Main Ring Road Highway Path SVG */}
          <svg className={`absolute inset-0 w-full h-full opacity-70 pointer-events-none ${
            lightMode ? 'text-slate-350' : 'text-slate-800/40'
            ...
            <text x="220" y="270" fill={lightMode ? "#64748B" : "#475569"} fontSize="9" className="font-mono rotate-45">WARDHA ROAD NH-44</text>
            {/* Besa Ring Road Connection */}
            <path d="M50 150 C 200 150, 300 200, 350 250" stroke="currentColor" strokeWidth="3" strokeDasharray="3" />
            <text x="70" y="140" fill={lightMode ? "#64748B" : "#475569"} fontSize="8" className="font-mono">BESA ROAD CORRIDOR</text>
          </svg>
              ...
              >
                <div className={`relative flex items-center justify-center p-2 rounded-full transition-all duration-300 ${
                  isSelected 
                    ...
                    : `${lightMode ? 'bg-white border-slate-300 text-blue-600 shadow-sm hover:bg-slate-50' : 'bg-slate-900 border-slate-705 text-blue-400'} border group-hover/marker:scale-110`
                }`}>
                ...
                <div className={`absolute left-9 border text-[10px] font-semibold py-1 px-2.5 rounded-lg whitespace-nowrap shadow-xl transition-all duration-300 ${
                  isSelected 
                    ? `opacity-100 translate-x-0 ${lightMode ? 'bg-white border-blue-500 text-slate-900 shadow-md' : 'border-blue-500 bg-slate-950/90 text-white'}` 
                    : `opacity-45 group-hover/marker:opacity-100 translate-x-1 ${lightMode ? 'bg-white border-slate-200 text-slate-800 shadow-sm' : 'border-slate-800 bg-[#091522] text-slate-300'}`
                }`}>
                  {proj.name}
                  <div className={`text-[8px] font-light block-span ${lightMode ? 'text-slate-550' : 'text-slate-400'}`}>{proj.location}</div>
                </div>
              </button>
            );
          })}
          ...
          <div className={`relative mt-auto border rounded-xl p-3 relative z-30 flex items-center justify-between transition-all duration-300 ${
            lightMode ? 'bg-white/95 border-slate-205 shadow-md text-slate-800' : 'bg-slate-900/95 border-slate-800/80 text-white'
          }`}>
            <div className={`flex gap-4 text-[10px] ${lightMode ? 'text-slate-600' : 'text-slate-400'}`}>
            ...
            <span className={`text-[9px] font-mono border px-1.5 py-0.5 rounded uppercase ${
              lightMode ? 'bg-slate-100 border-slate-200 text-blue-600' : 'bg-slate-950 border-slate-800 text-sky-400'
            }`}>
              Live Vector Map API
            </span>
          </div>
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          ...
          {selectedMapProject && (
            <div className={`border rounded-2xl p-5 relative transition-all duration-300 ${
              lightMode ? 'bg-[#F1F5F9] border-slate-200 shadow-sm' : 'bg-slate-950 border-slate-800/80'
            }`}>
              <div className="flex justify-between items-start gap-4">
                <div>
                  <span className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded font-mono ${
                    lightMode ? 'bg-slate-200 text-slate-700' : 'bg-slate-800 text-slate-300'
                  }`}>
                    {selectedMapProject.type.toUpperCase()}
                  </span>
                  <h4 className={`text-lg font-serif tracking-tight font-bold mt-1.5 ${
                    lightMode ? 'text-slate-900' : 'text-white'
                  }`}>
                    {selectedMapProject.name}
                  </h4>
                  <p className={`text-xs flex items-center gap-1 mt-0.5 ${lightMode ? 'text-slate-600' : 'text-slate-400'}`}>
                    <Navigation className="w-3.5 h-3.5 text-blue-500 inline" /> {selectedMapProject.location} | Hub: {selectedMapProject.mapHotspot}
                  </p>
                </div>
                <div className="text-right">
                  <div className={`text-[10.5px] uppercase font-mono ${lightMode ? 'text-slate-500' : 'text-slate-440'}`}>Investment Pricing</div>
                  <div className={`text-base font-bold font-mono ${lightMode ? 'text-red-655 font-extrabold' : 'text-red-400'}`}>{selectedMapProject.priceRange}</div>
                </div>
              </div>

              <p className={`text-xs mt-3 leading-relaxed ${lightMode ? 'text-slate-600' : 'text-slate-300'}`}>
                {selectedMapProject.description}
              </p>

              {/* Specs Badge Bar */}
              <div className={`grid grid-cols-2 sm:grid-cols-3 gap-3 mt-4 pt-4 border-t text-[11px] ${
                lightMode ? 'border-slate-200' : 'border-slate-800/60'
              }`}>
                {selectedMapProject.specs.slice(0, 3).map((spec, i) => (
                  <div key={i} className={`p-2 rounded-lg border transition-all ${
                    lightMode ? 'bg-white border-slate-200 shadow-sm' : 'bg-slate-900/60 border-slate-800/50'
                  }`}>
                    <div className="text-slate-500 font-mono text-[9px] uppercase">{spec.label}</div>
                    <div className={`font-semibold mt-0.5 ${lightMode ? 'text-slate-900' : 'text-white'}`}>{spec.value}</div>
                  </div>
                ))}
              </div>
              ...
              <div className={`flex justify-between items-center mt-6 pt-4 border-t ${
                ...
                <button
                  ...
                  className={`text-xs font-semibold hover:text-blue-500 transition-colors flex items-center gap-1 group ${
          ...
          {selectedMapProject && (selectedMapProject.type === 'plot') ? (
            <div className={`border rounded-2xl p-5 ${
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-4">
                <div>
                  <h5 className={`text-xs font-semibold tracking-wider uppercase font-mono ${
                ...
                <div className={`flex items-center gap-1 text-[10px] font-mono ${lightMode ? 'text-slate-600' : 'text-slate-400'}`}>
              ...
              <div className={`grid grid-cols-6 sm:grid-cols-10 gap-1.5 p-3 rounded-xl max-h-[140px] overflow-y-auto custom-scrollbar ${
                lightMode ? 'bg-slate-200/50' : 'bg-slate-900'
              }`}>
                {plotsData.map((plot) => {
                  let colorClass = 'bg-emerald-600/30 border-emerald-500/50 hover:bg-emerald-600/60 text-emerald-300';
                  if (plot.status === 'booked') colorClass = lightMode ? 'bg-slate-300/60 border-slate-350 text-slate-405' : 'bg-slate-800 border-slate-700 text-slate-500 cursor-not-allowed';
